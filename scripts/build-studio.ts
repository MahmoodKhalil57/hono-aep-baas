...
// Embed the built shell HTML so the worker can serve the ONE engine at any
// (nested) interface route — /v1/projects/{p}[/…]/{studio|admin}. The
// <script>/chunk refs inside are absolute, served by Workers Static Assets.
{
  const shell = await Bun.file("dist/studio-assets/studio.html").text();
  await Bun.write(
    "src/server/generated/studio-shell.ts",
    `// GENERATED by scripts/build-studio.ts — the built engine shell.\nexport const studioShellHtml = ${JSON.stringify(shell)};\n`,
  );
  console.log("embedded studio shell → src/server/generated/studio-shell.ts");
}
